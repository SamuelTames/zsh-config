...
local user_host="%B${fg_green}%n@%M%b%f${reset_color}"
local the_prompt_sign="%(?.${fg_white}.${fg_red})›${reset_color} "

export PROMPT="${user_host} ${the_prompt_sign}"

# precmd is called just before the prompt is printed
precmd () {
  RPROMPT=""
  setopt promptsubst
  print -Pn "${reset_color}\e]2; %~/ \a"
}
